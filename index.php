<style>html,body{font-family: consolas;letter-spacing: 1px} </style>
<?php

///////////////////Convertimos el mensaje y la contraseña de hexadecimal a binario
echo "<h1>Convertimos el mensaje y la contraseña de hexadecimal a binario</h1>";
//hexa-bin
...
///////////////////Ciframos el mensaje
echo "<h1>Ciframos el mensaje</h1>";
echo "<h3>Dividimos el mensaje en L y R</h3>";
$l=substr($m_bin,0,32);
...
echo "<h1>Planificación de contraseña</h1>";
...
echo "C<sub>0</sub>: ".$c."</br>";
echo "D<sub>0</sub>: ".$d."</br>";
echo "</br>";
...
for ($i=1, $j=0;$i<=count($iteraciones);$i++,$j++) {
    ...
    echo "<tr><td>C<sub>$i</sub></td><td>".$c_iteraciones["c".$i]."</td>";

    $a = substr($d_iteraciones["d".$j],0, $iteraciones[$j]);
    $b = substr($d_iteraciones["d".$j],$iteraciones[$j],28);
    $d_iteraciones["d".$i] = $b.$a;

    echo "<td>D<sub>$i</sub>: </td><td>".$d_iteraciones["d".$i]."</td></tr>";
}
echo "</table>";

//permutación 2

echo "<h3>Permutación 2</h3>";
...
echo "</br>";
for ($i=1;$i<=count($k_iteraciones);$i++) {
    echo "K<sub>$i</sub>: ".$k_iteraciones["k".$i]."</br>";
}

echo "<h1>Ciframos el mensaje</h1>";

echo "<h3>Permutación 1</h3>";
